...
export default class Launchpad extends EventEmitter {
  ...
  async getTypeMappings(): Promise<Mapping | undefined> {
    if (this.mapping != undefined) return this.mapping;
  ...
  midiMessage = async (event: any) => {
    ...
    if (data[2] === mapping.pressedState[0]) {
      ...
      console.error(
        `Invalid pressed state received from ${this.name}, expected ${mapping.pressedState}, received ${data[2]}`,
      );
    }
  };

  async getMessageCodes(
    item1: number,
    item2: number,
    ...
    for (const sectionName in mapping.gridMappings) {
      ...
      for (const item of mapping.gridMappings[sectionName]) {
        if (item.x === 0 && item.y === 0) currentSection = item.number;
  ...
  async getButtonCombo(
    section: Section,
    x: number,
    y: number,
  ): Promise<(number | undefined)[]> {
    return [
      await this.getSectionNumber(section),
      await this.getButtonNumber(section, x, y),
    ];
  }

  async getButtonNumber(
    section: Section,
    x: number,
    y: number,
  ): Promise<number | undefined> {
    ...
    for (const item of mapping.gridMappings[section]) {
      if (item.x === x && item.y === y) return item.number;
    }
    return undefined;
  }
  ...
  async getSectionNumber(section: Section): Promise<number | undefined> {
    const mapping = await this.getTypeMappings();
    for (const item of mapping.gridMappings[section]) {
      if (item.x === 0 && item.y === 0) return item.number;
    }
    return undefined;
  }
        ...
        if (item.x !== 0 || item.y !== 0) {
          this.output.sendSysex(
            ...
            mapping.sysExSequence.concat(
              [0x0b, item.number].concat(color.toRgb6Array()),
            ),
          );
        }
      }
  ...
  async setColor(section: Section, x: number, y: number, color: Color) {
    ...
    if (buttonNumber === undefined)
      ...
      this.output.sendSysex(
        [],
        mapping.sysExSequence.concat(
          [0x0b, buttonNumber].concat(color.toRgb6Array()),
        ),
      );
    }
  }
        ...
        if (
          ...
          [0, 0x20, 0x29, 0].reduce(
            (previous, current, index) =>
              previous && eventData[index + 1] === current,
            true,
          ) &&
          eventData[5] === 0x70
        ) {
