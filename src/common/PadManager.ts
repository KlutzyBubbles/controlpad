...
export class PadManager extends EventEmitter {
  ...
  addLaunchpad(name: string) {
    if (this.hasLaunchpad(name)) return;
    if (this.selectedDevice == undefined) this.selectedDevice = name;
  ...
  removeLaunchpad(name: string) {
    if (!this.hasLaunchpad(name)) return;
    if (this.selectedDevice == name) this.selectedDevice = undefined;
  ...
  portNeutralize = (x: string) =>
    x.toUpperCase().split('IN').join('').split('OUT').join('');
  portsMatch = (input: string, output: string) =>
    this.portNeutralize(input) === this.portNeutralize(output);
